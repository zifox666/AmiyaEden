...
- 管理员可查看全量队长绩效，列表与详情都会展示队长主角色名和昵称，并可手动触发赏金归因同步与奖励处理
- `帮扶管理` 页面新增 `奖励发放历史` tab，按“每次处理、每名队长一行”的粒度展示历史结算结果
- `帮扶管理` 页面新增 `关系变更历史` tab，可按变更时间、队长用户 ID、新人角色 ID 查看全量关系记录，并显示实际创建该关系的角色
...
- 只要任一角色 `total_sp >= 20,000,000`，用户就不再属于新人
- `total_sp >= 10,000,000` 的绑定角色数达到 `3`，用户也不再属于新人
...
- `max_character_sp` 控制“任一角色技能点毕业线”
- `multi_character_sp` 与 `multi_character_threshold` 共同控制“多角色技能点毕业线”
...
- 玩家侧会遍历该新人用户下的全部角色钱包流水
- 队长侧只看该队长用户“归因作业执行时”的当前主角色
- 玩家流水只把 `bounty_prizes` 作为归因输入
- 需要满足：
  - 该玩家流水发生时存在 active 的队长关联
  - 玩家与队长流水在同一 `system_id`
  - 双方 `reason` 字段解析出的 NPC ID 与数量一致
  - 两条流水时间差不超过 `15` 分钟
...
- 绩效页展示全部队长的概览排行，并支持查看单个队长详情
- 奖励发放历史页展示汇总卡片和按处理批次展开的结算记录
- 关系变更历史页支持按变更日期、队长角色名或昵称、新人角色名或昵称过滤
- 关系变更历史页名称筛选不区分大小写
...
- `我是队长` 页面要求真实角色 `captain`
- `帮扶管理` 页面要求 `admin`
- 后端服务层会再次校验资格或角色，前端菜单隐藏只属于 UX
