...
- `static/src/views/info/npc-kills` — 个人报表（单角色 / 全角色切换）
- `static/src/views/system/npc-kills` — 管理员公司报表

### 后端路由

- `POST /api/v1/info/npc-kills` — 个人单角色报表
- `POST /api/v1/info/npc-kills/all` — 个人全角色汇总
...
- 个人接口在服务层校验 `character_id` 归属，非本人角色返回错误
...
| `character_name` | 全角色汇总和公司报表时填充 |

## UI 呈现

### 个人页面

1. 角色选择器（下拉，含头像）+ 「所有角色」选项 + 日期范围选择
...
- 个人接口强制校验角色归属，不可跨用户查询
- 公司接口只涵盖当前已绑定有效 token 的角色
